...
{
    public class CannyEdgeDetection : CustomEffectBase
    {
        ...
        public CannyEdgeDetection(IImageProvider source)
            : base(source)
        {
        }
        ...
        {
            Width = (int)sourcePixelRegion.Bounds.Width;
            Height = (int)sourcePixelRegion.Bounds.Height;
            ...
            for (i = 0; i <= (Width - 1); i++)
                for (j = 0; j <= (Height - 1); j++)
                {
                    targetPixelRegion.ImagePixels[j * Width + i] = EdgeMap[i, j] == 1 ? white : black;
                }
